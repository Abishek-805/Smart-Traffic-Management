...
async def get_version():
    return wrap_response(SystemVersionInfo())

@router.get("/build", response_model=ApiResponse[Dict[str, str]])
async def get_build_info():
    return wrap_response({
        "version": "2.0.0",
        "backend": "FastAPI App Backend (Port 8000)",
        "websocket_server": "FastAPI WebSocket Server (Port 8001)",
        "frontend": "React SPA",
        "protocol": "1.0",
...
async def configure_camera(direction: str, payload: Dict[str, Any] = Body(...)):
    log_svc.add_log(
        level="INFO", category="SYSTEM", component="CameraService",
...
async def get_mobile_nodes():
    ...
    return wrap_response(MobileNodesData(summary=summary, nodes=nodes_list))
...
async def generate_qr_code(direction: str = Query("north")):
