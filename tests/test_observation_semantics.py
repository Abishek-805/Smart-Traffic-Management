...
def _vehicle(*, x=10, timestamp=1.0, observation_state=ObservationState.OBSERVED):
    return Detection(
...
def test_tracker_marks_detector_evidence_and_projection_explicitly():
    ...
    assert observed[0].observation_state == ObservationState.OBSERVED
    assert predicted[0].observation_state == ObservationState.PREDICTED
    assert predicted[0].to_dict()["observation_state"] == "PREDICTED"
...
def test_predictions_do_not_confirm_or_refresh_vehicle_state():
    ...
    manager.update(
        [_vehicle(timestamp=1.1, observation_state=ObservationState.PREDICTED)],
        frame_number=11,
        timestamp=1.1,
    )
...
def test_only_confirmed_observations_feed_counts_and_queue_analytics():
    ...
    manager.update(
        [_vehicle(timestamp=1.2, observation_state=ObservationState.PREDICTED)],
        frame_number=12,
        timestamp=1.2,
    )
